...
//					Toast.makeText(getApplicationContext(), "Permission denied", Toast.LENGTH_SHORT).show();
    ...
    private void setResultSlider() {
        ...
        switch (itemVideo.getLatestVideoType()) {
            case "local":
                Glide.with(ActivityVideoDetails.this).load(itemVideo.getLatestVideoImgBig()).placeholder(R.drawable.loading).centerCrop().into(imageViewVideo);
                break;
            case "server_url":
                Glide.with(ActivityVideoDetails.this).load(itemVideo.getLatestVideoImgBig()).placeholder(R.drawable.loading).centerCrop().into(imageViewVideo);
                break;
            case "youtube":
                Glide.with(ActivityVideoDetails.this).load(Constant.YOUTUBE_IMAGE_FRONT + itemVideo.getLatestVideoPlayId() + Constant.YOUTUBE_SMALL_IMAGE_BACK).placeholder(R.drawable.loading).centerCrop().into(imageViewVideo);
                break;
            case "dailymotion":
                Glide.with(ActivityVideoDetails.this).load(Constant.DAILYMOTION_IMAGE_PATH + itemVideo.getLatestVideoPlayId()).placeholder(R.drawable.loading).centerCrop().into(imageViewVideo);
                break;
            case "vimeo":
                Glide.with(ActivityVideoDetails.this).load(itemVideo.getLatestVideoImgBig()).placeholder(R.drawable.loading).centerCrop().into(imageViewVideo);
                break;
